...
import { Upload, FileAudio, CheckCircle, AlertCircle, Loader2, X, RefreshCw } from 'lucide-vue-next'
...
const startListening = (taskId) => {
  ...
  eventSource.value.onmessage = async (event) => {
    ...
    if (data.type === 'summary_chunk') {
      ...
      currentTask.value.summary = (currentTask.value.summary || '') + data.content
...
<template>
  <div class="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
    <header class="text-center mb-12">
      <h1 class="text-5xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400 mb-4">
        STT Summarizer
      </h1>
      <p class="text-lg text-slate-400">
        Upload audio, get transcription & AI summary — powered by a decoupled pipeline.
      </p>
    </header>
    ...
    <main class="space-y-8">
      ...
      <div 
        ...
        :class="[
          ...
          isDragging ? 'border-indigo-500 bg-indigo-500/10' : 'border-slate-700 hover:border-slate-500 hover:bg-slate-800/10'
        ]"
      >
        <input 
          type="file" 
        ...
        <div class="flex flex-col items-center">
          <div class="p-4 bg-indigo-500/20 rounded-2xl mb-6 group-hover:scale-110 transition-transform">
      ...
      >
        <div v-if="currentTask || isUploading" class="bg-slate-800/40 backdrop-blur-xl border border-slate-700/50 rounded-3xl p-8 shadow-2xl">
          <div class="flex items-start justify-between mb-8">
            <div class="flex items-center gap-4">
              ...
              <div>
                <h3 class="text-xl font-semibold text-slate-100 truncate max-w-xs">
                ...
                <div class="flex items-center gap-2 mt-1">
                  <span 
                    :class="[
                      ...
                      currentTask?.status === 'failed' || currentTask?.status === 'cancelled' ? 'bg-rose-500/20 text-rose-400' :
            ...
            <button 
              v-if="!['completed', 'failed', 'cancelled'].includes(currentTask?.status) && !isUploading"
              @click="cancelTask"
          ...
          <div class="space-y-3">
            ...
            <div class="h-1.5 w-full bg-slate-700 rounded-full overflow-hidden">
              <div 
                ...
                :style="{ width: `${isUploading ? uploadProgress : currentTask?.progress}%` }"
              ></div>
            </div>
          </div>

          <!-- Streaming Summary (visible during processing when chunks arrive) -->
          <div v-if="currentTask?.summary && currentTask?.status === 'processing'" class="mt-8 space-y-3 animate-in">
            <h4 class="text-sm font-bold text-slate-500 uppercase flex items-center gap-2">
              <Loader2 class="w-4 h-4 animate-spin" /> 摘要生成中
            </h4>
            <div class="bg-indigo-500/5 rounded-2xl p-6 border border-indigo-500/20 text-indigo-50/90 leading-relaxed whitespace-pre-wrap">
              {{ currentTask.summary }}<span class="animate-pulse">▊</span>
            </div>
          </div>

          <!-- Final Results -->
          <div v-if="currentTask?.status === 'completed'" class="mt-10 grid gap-6 animate-in">
            <div class="space-y-3">
              <h4 class="text-sm font-bold text-slate-500 uppercase flex items-center gap-2">
          ...
          <!-- Error Message -->
          <div v-if="currentTask?.status === 'failed' || currentTask?.status === 'cancelled'" class="mt-6 flex items-center gap-3 p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl text-rose-400">
